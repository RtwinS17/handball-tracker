...
router.post('/', async (req, res) => {
    ...
    try {
        const conn = await pool.getConnection();
        const result = await conn.query('INSERT INTO children (first_name, last_name, birth_year) VALUES (?, ?, ?)', [first_name, last_name, birth_year]);
        conn.release();
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const conn = await pool.getConnection();
        const result = await conn.query('UPDATE children SET first_name = ?, last_name = ?, birth_year = ? WHERE id = ?', [first_name, last_name, birth_year, id]);
        conn.release();
